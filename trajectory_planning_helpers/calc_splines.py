...
DOWNSCALE = 3.0  # downscale factor for normal vectors to avoid normal vectors intersecting in every corner (default: 3.0)
...
def calc_splines(path: np.ndarray,
    ...
    norm_factors = (1.0 / np.sqrt(np.sum(np.power(normvec, 2), axis=1))) / DOWNSCALE
